...
<div class="page-header">
    <div><h1>Waiting Room</h1><p>Manage live patient queue</p></div>
    <span class="live-badge"><span class="live-dot"></span> LIVE — <?= count($queue) ?> in queue</span>
</div>
<?php if(empty($queue)): ?>
<div class="card" style="text-align:center;padding:60px;color:var(--gray)"><i class="fas fa-door-open" style="font-size:40px;opacity:.3;margin-bottom:12px"></i><h3>Queue is empty</h3></div>
<?php else: ?>
    ...
    <div class="card" style="padding:16px;border-left:4px solid <?= $active?'var(--primary)':'var(--border)' ?>">
        <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap">
            ...
            <div style="flex:1">
                ...
                <div style="font-size:13px;color:var(--gray)">Dr. <?= htmlspecialchars($q['doc']['name']??'') ?></div>
                <div style="font-size:12px;color:var(--gray-light)">Checked in: <?= fmtDate($q['check_in_time'],'h:i A') ?></div>
            </div>
